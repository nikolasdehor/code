  ...
  if (feature('SSH_REMOTE') && _pendingSSH) {
    ...
    if (rawCliArgs[0] === 'ssh' && rawCliArgs[1] && !rawCliArgs[1].startsWith('-')) {
      ...
      if (rest.includes('-p') || rest.includes('--print')) {
        process.stderr.write('Error: headless (-p/--print) mode is not supported with verboo ssh\n');
...
async function run(): Promise<CommanderCommand> {
  ...
  .option('--plugin-dir <path>', 'Load plugins from a directory for this session only (repeatable: --plugin-dir A --plugin-dir B)', (val: string, prev: string[]) => [...prev, val], [] as string[]).option('--disable-slash-commands', 'Disable all skills', () => true).option('--chrome', 'Enable Verboo in Chrome integration').option('--no-chrome', 'Disable Verboo in Chrome integration').option('--file <specs...>', 'File resources to download at startup. Format: file_id:relative_path (e.g., --file file_abc:doc.txt file_def:img.png)').option('--list-models', 'List available Verboo models and exit').action(async (prompt, options) => {
    ...
    if (prompt === 'code') {
      ...
      console.warn(chalk.yellow('Tip: You can launch Verboo Code with just `verboo`')); // VERBOO-BRAND
      prompt = undefined;
    }
    ...
    if (enableClaudeInChrome) {
      ...
      try {
        ...
        logError(error);
        // biome-ignore lint/suspicious/noConsole:: intentional console output
        console.error(`Error: Failed to run with Verboo in Chrome.`);
        process.exit(1);
      }
    ...
    if (options.continue) {
      ...
      if (!targetSessionId) {
        ...
        if (sessions.length === 0) {
          ...
          return await exitWithMessage(root, `Assistant installed in ${installedDir}. The daemon is starting up — run \`verboo assistant\` again in a few seconds to connect.`, {
            exitCode: 0,
      ...
      if (remote !== null) {
        ...
        if (!isRemoteTuiEnabled) {
          ...
          process.stdout.write(`Resume with: verboo --teleport ${createdSession.id}\n`);
          await gracefulShutdown(0);
          process.exit(0);
        }
            ...
            if (repoValidation.status === 'mismatch' || repoValidation.status === 'not_in_repo') {
              ...
              if (sessionRepo) {
                ...
                if (existingPaths.length > 0) {
                  ...
                  throw new TeleportOperationError(`You must run verboo --teleport ${teleport} from a checkout of ${sessionRepo}.`, chalk.red(`You must run verboo --teleport ${teleport} from a checkout of ${chalk.bold(sessionRepo)}.\n`));
                }
              }
  ...
  if ("external" === 'ant') {
    ...
    program.option('--agent-teams', '[internal-only] Force Verboo to use multi-agent mode for solving problems', () => true);
  }
  if (feature('TRANSCRIPT_CLASSIFIER')) {
  ...
  mcp.command('serve').description(`Start the Verboo Code MCP server`).option('-d, --debug', 'Enable debug mode', () => true).option('--verbose', 'Override verbose mode setting from config', () => true).action(async ({
    debug,
    verbose
  }: {
    debug?: boolean;
  ...
  mcp.command('add-from-claude-desktop').description('Import MCP servers from Verboo Desktop (Mac and WSL only)').option('-s, --scope <scope>', 'Configuration scope (local, user, or project)', 'local').action(async (options: {
    scope?: string;
  }) => {
    const {
  ...
  if (feature('DIRECT_CONNECT')) {
    program.command('server').description('Start a Verboo Code session server').option('--port <number>', 'HTTP port', '0').option('--host <string>', 'Bind address', '0.0.0.0').option('--auth-token <token>', 'Bearer token for auth').option('--unix <path>', 'Listen on a unix domain socket').option('--workspace <dir>', 'Default working directory for sessions that do not specify cwd').option('--idle-timeout <ms>', 'Idle timeout for detached sessions in ms (0 = never expire)', '600000').option('--max-sessions <n>', 'Maximum concurrent sessions (0 = unlimited)', '32').action(async (opts: {
      ...
      if (existing) {
        process.stderr.write(`A Verboo Code server is already running (pid ${existing.pid}) at ${existing.httpUrl}\n`);
        process.exit(1);
      }
  ...
  if (feature('DIRECT_CONNECT')) {
    program.command('open <cc-url>').description('Connect to a Verboo Code server (internal — use cc:// URLs)').option('-p, --print [prompt]', 'Print mode (headless)').option('--output-format <format>', 'Output format: text, json, stream-json', 'text').action(async (ccUrl: string, opts: {
  ...
  auth.command('logout').description('Log out from your Verboo account').action(async () => {
    const {
      authLogout
    } = await import('./cli/handlers/auth.js');
    await authLogout();
  ...
  const pluginCmd = program.command('plugin').alias('plugins').description('Manage Verboo Code plugins').configureHelp(createSortedHelpConfig());
  ...
  const marketplaceCmd = pluginCmd.command('marketplace').description('Manage Verboo Code marketplaces').configureHelp(createSortedHelpConfig());
  ...
  // Setup token command
  program.command('setup-token').description('Set up a long-lived authentication token (requires Verboo subscription)').action(async () => {
    const [{
      setupTokenHandler
    }, {
      createRoot
  ...
  if (feature('KAIROS')) {
    program.command('assistant [sessionId]').description('Attach the REPL as a client to a running bridge session. Discovers sessions via API if no sessionId given.').action(() => {
      ...
      process.stderr.write('Usage: verboo assistant [sessionId]\n\n' + 'Attach the REPL as a viewer client to a running bridge session.\n' + 'Omit sessionId to discover and pick from available sessions.\n');
      process.exit(1);
    });
  }

  // Doctor command - check installation health
  program.command('doctor').description('Check the health of your Verboo Code auto-updater. Note: The workspace trust dialog is skipped and stdio servers from .mcp.json are spawned for health checks. Only use this command in directories you trust.').action(async () => {
    const [{
      doctorHandler
    }, {
      createRoot
  ...
  // claude install
  program.command('install [target]').description('Install Verboo Code native build. Use [target] to specify version (stable, latest, or specific version)').option('--force', 'Force installation even if already installed').action(async (target: string | undefined, options: {
    force?: boolean;
  }) => {
    const {
      installHandler
