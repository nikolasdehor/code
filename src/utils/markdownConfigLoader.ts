...
// Verboo configuration directory names
 ...
 * stopping there makes the parent project's `.verboo/` unreachable (#31905).
 *
 ...
 *
 * Worktrees (under `.verboo/worktrees/`) stay on the old behavior: their `.git`
 ...
 * collecting all .verboo directories along the way.
 *
 * Stopping at git root prevents commands/skills from parent directories outside the repository
 * from leaking into projects. For example, if ~/projects/.verboo/commands/ exists, it won't
 ...
 * @returns Array of directory paths containing .verboo/subdir, from most specific (cwd) to least specific
 */
...
export const loadMarkdownFilesForSubdir = memoize(
  async function (
    ...
    const managedDir = join(getManagedFilePath(), '.verboo', subdir)
    const projectDirs = getProjectDirsUpToHome(subdir, cwd)

    // For git worktrees where the worktree does NOT have .verboo/<subdir> checked
    ...
    //
    // Only add the main repo's copy when the worktree root's .verboo/<subdir>
    // is absent. A standard `git worktree add` checks out the full tree, so the
    // worktree already has identical .verboo/<subdir> content — loading the main
    ...
    // This prevents the same file from appearing multiple times when ~/.verboo is
 ...
 * @param dir Directory (eg. "~/.verboo/commands")
 * @returns Array of parsed markdown files with metadata
 */
